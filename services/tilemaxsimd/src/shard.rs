...
impl HostCache {
    ...
    fn put(&mut self, key: String, payload: Vec<u8>) {
        ...
        if let Some(previous) = self.entries.remove(&key) {
            self.current_bytes -= previous.payload.len();
        }
        ...
        while self.current_bytes + payload.len() > self.maximum_bytes {
            ...
            let victim = self
                .entries
                .remove(&victim_key)
                .expect("host victim disappeared");
            self.current_bytes -= victim.payload.len();
        ...
        self.entries.insert(key, HostEntry { payload, priority });
    }
}
...
impl ShardStore {
    pub fn open(
        ...
        Ok(Self {
            contracts,
            host_cache: HostCache::new(host_cache_bytes),
            batch_read_calls: 0,
            batch_read_bytes: 0,
            verify_full_shards,
        })
    ...
    pub fn resolve_many(&mut self, descriptors: &[Descriptor]) -> Result<Vec<Vec<u8>>> {
        ...
        for ((contract_name, shard_name), mut entries) in groups {
            ...
            for (output_index, payload) in resolved {
                output[output_index] = Some(payload.clone());
                self.host_cache
                    .put(cache_key(&descriptors[output_index]), payload);
            }
        }
        output
            .into_iter()
