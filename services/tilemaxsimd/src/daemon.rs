...
#[derive(Parser)]
#[command(about = "Native TileMaxSim shard, cache, and accelerator scheduling daemon")]
struct Args {
    #[arg(long)]
    socket: PathBuf,
...
pub fn run_with_backend_factory<F>(factory: F) -> Result<()>
where
    F: Fn(i32, usize, usize) -> Result<Box<dyn AcceleratorBackend>>,
{
    let args = Args::parse();
