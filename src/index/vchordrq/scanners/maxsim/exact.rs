...
const MAX_EXPLICIT_CANDIDATES: usize = 65_536;
...
/// Half-precision overload of exact candidate-only TileMaxSim.
#[pg_extern(sql = "")]
...
fn collect_candidate_ids(candidate_ids: Array<'_, i64>) -> Result<Vec<i64>, RerankError> {
    ...
    if result.is_empty() || result.len() > MAX_EXPLICIT_CANDIDATES {
        return Err(RerankError::Configuration(
            "candidate_ids must contain between 1 and 65536 IDs",
        ));
    }
    Ok(result)
}
...
mod tests {
    ...
    #[test]
    fn explicit_candidates_are_bounded_and_unique() {
        ...
        let last = candidate_for_ordinal(65_535).unwrap().heap_key;
